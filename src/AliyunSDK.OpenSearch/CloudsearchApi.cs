    ...
    public class CloudsearchApi
    {
        ...
        private string _makeSignAliyun(Dictionary<string, object> parameter, string method)
        {
            ...
            if (parameter != null)
            {
                ...
                q = percentEncode(q);
                q = System.Net.WebUtility.UrlEncode(q);
                q = percentEncode(q);
            ...
            NameValueCollection httpOptions = null)
        {
            ...
            if (debug)
            {
                ...
                debugInfo.Add(string.Format("decodedQueryString:   {0}", System.Net.WebUtility.UrlDecode(url)));
            }
...
#if NETCORE
                    var success = socket.ConnectAsync(ub.Host, ub.Port).Wait(timeout);
                    if (!socket.Connected)
                    {
